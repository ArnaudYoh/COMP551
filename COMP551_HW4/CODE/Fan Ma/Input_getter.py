...
def main(data_size): # number of data points clustered into a single one. 
	...
	neg = list(csv.DictReader(open("clean_North_negative_gradient.csv", "r")))
	pos_lab = list(csv.DictReader(open("clean_North_positive_labels.csv", "r")))
	pos = list(csv.DictReader(open("clean_North_positive_gradient.csv", "r")))
			...
			if data["label"]=="1":
				...
				for k in range(data_size): 
					...
					for feat in features:
						if (feat == 'timestamp'):
							float_days = date2num(datetime.datetime.strptime(data_line[feat], '%m/%d/%Y'))
							matrix_row.append(float_days)	
						else:
