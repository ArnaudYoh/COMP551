...
def check_neighbor(image, loc, square=3, threshold=230):
    ...
    for i in range(-square, square):
        for j in range(-square, square):
            if x + i >= 0 and x + i < 28 and y + j >= 0 and y + j < 28 and (i != 0 or j !=0):
                total += 1
...
#cla = linreg.load_cla('arnaudclanewton2.pk1')
results = cla.predict(X_test.reshape(X_test.shape[0], X_train.shape[1] ** 2))
